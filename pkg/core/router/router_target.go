...
type PathTarget struct {
	Config        *config.TargetConfig  // Configuration for the target
...
func NewPathTarget(cfg *config.TargetConfig) (*PathTarget, error) {
	...
	for _, fCfg := range cfg.Filters {
		...
		if err != nil {
			return nil, fmt.Errorf("error on target (%s): %w", pt.Config.Name, err)
		}
	...
	if fn, ok := filterStrategyMap[cfg.FilterStrategy]; ok {
		...
		return nil, fmt.Errorf("error on target (%s): unhandled filter strategy (%s)", pt.Config.Name, cfg.FilterStrategy)
	}
	...
	if fn, ok := actionRequestMap[cfg.RequestAction]; ok {
		...
		return nil, fmt.Errorf("error on target (%s): unhandled request action (%s)", pt.Config.Name, cfg.RequestAction)
	}
	...
	if fn, ok := requestStrategyMap[cfg.RequestStrategy]; ok {
		...
		return nil, fmt.Errorf("error on target (%s): unhandled request strategy (%s)", pt.Config.Name, cfg.RequestStrategy)
	}
...
func (pt *PathTarget) ActionRequest_Forward(w http.ResponseWriter, r *http.Request, body []byte) {
	...
	service := backend.GetBackendService(pt.Config.TargetService)
	if service == nil {
		context.ReturnResponseText(w, r, http.StatusInternalServerError, fmt.Sprintf("No handler available for target service: %s", pt.Config.TargetService))
		return
	}
	resp, err := pt.UseStrategy(w, r, body, service)
	if err != nil {
		context.ReturnResponseText(w, r, http.StatusInternalServerError, fmt.Sprintf("Failed to route request to target service (%s): %s", pt.Config.TargetService, err))
		return
	}
...
func (pt *PathTarget) UseStrategy_Ping(w http.ResponseWriter, r *http.Request, body []byte, service *backend.BackendService) (*context.ResponseData, error) {
	...
	if err != nil {
		pt.LogError("error on connection", err, service.Config.Name, nil, 0)
		return nil, err
	}
	context.AppendToContextTrace(r, "connection", conn.Config.Name)

	// Send the request to the connection
	response, err := conn.SendRequest(r.Method, r.URL.Path, r.Header, pt.Headers, r.URL.Query(), body)
	if err != nil {
		pt.LogError("error on request", err, service.Config.Name, nil, 0)
		return nil, err
	}
	if response.StatusCode/100 != 2 {
		pt.LogError("error on response", nil, service.Config.Name, response.Body, response.StatusCode)
	}
...
func (pt *PathTarget) UseStrategy_Primary(w http.ResponseWriter, r *http.Request, body []byte, service *backend.BackendService) (*context.ResponseData, error) {
	...
	primary := connections[service.Config.Members[0]] // Get the primary connection based on the first member in the service
	if primary == nil {
		return nil, fmt.Errorf("no primary connection found for service: %s", service.Config.Name)
	}
	for _, conn := range connections {
		go func(conn *backend.BackendConnection) {
			...
			if conn.Config.Name == primary.Config.Name {
...
func (pt *PathTarget) ForwardToReplica(w http.ResponseWriter, r *http.Request, body []byte) {
	if len(pt.Config.TargetReplica) == 0 {
		return
	}
	replica := backend.GetBackendService(pt.Config.TargetReplica)
	if replica == nil {
		pt.LogError("error on replica", fmt.Errorf("no service handler found"), pt.Config.TargetReplica, nil, 0)
	} else {
		for _, member := range replica.Config.Members {
			conn := backend.GetBackendConnection(member)
			if conn == nil {
				pt.LogError("error on replica", fmt.Errorf("no connection found for member: %s", member), pt.Config.TargetReplica, nil, 0)
				continue
			}
			go func(c *backend.BackendConnection) {
				resp, err := c.SendRequest(r.Method, pt.Config.UpstreamEndpoint, r.Header, pt.Headers, r.URL.Query(), body)
				if err != nil {
					pt.LogError("error on replica", err, c.Config.Name, nil, 0)
				} else if resp.StatusCode/100 != 2 {
					pt.LogError("error on replica", nil, c.Config.Name, resp.Body, resp.StatusCode)
				}
			}(conn)
		}
	}
...
func (pt *PathTarget) LogError(prefix string, err error, service string, response []byte, status int) {
	...
	pt.TargetLogger.Error().Err(err).
		...
		Msg(fmt.Sprintf("error on target (%s):", pt.Config.Name))
}
